...
class CiudadanosHandler implements IDataBaseHandler
{
    ...
    {
        $stm = $this->connection->db->prepare('update Ciudadanos set cedula = ?, nombre = ?, apellido = ?, email = ? where id_ciudadano = ?');
        $tm->bind_param('isssi', $entity->cedula, $entity->nombre, $entity->apellido, $entity->email, $entity->$idCiudadano);
        $stm->execute();
    }
